...
# , get_unique_phrases
from .grounding_dino_predict import groundingdino_predict, prompt_to_dino_prompt, VAEencode_dnl13
...
class DinoSegmentationProcessor:
    ...
    def main(
        ...
        for index, item in tqdm(enumerate(image), total=len(image), desc=f"{dnl13} analyzing batch", unit="img", ncols=100, colour='green'):
            item_dino = Image.fromarray(np.clip(255. * item.cpu().numpy(), 0, 255).astype(np.uint8)).convert('RGBA')
            bbox, phrases, logits = groundingdino_predict(grounding_dino_model, item_dino, prompt, box_threshold, upper_confidence_threshold, lower_confidence_threshold, device)
            ...
            res_img_rgb.append(masked_image_with_bg)

            # Debug Image
            debug_image = createDebugImageTensor( item,  bbox, phrases, logits)
        ...
        if vae is not None:
            # we do this just for nicer progressbars prints 🤣
            ...
            for index, item in tqdm(enumerate(image), total=len(image), desc=f"{dnl13} generate Latent", unit="img", ncols=100, colour='#FFC0CB'):
                encoded_tensor = VAEencode_dnl13(vae=vae, image=item.unsqueeze(0), mask=res_masks[index], latent_grow_mask=latent_grow_mask, device=device)
            ...
            for index, item in enumerate(image):
                matching_entries = [d for d in data if d["frame"] == index]
                for entry in matching_entries:
                    entry["latent"] =  {"samples":torch.zeros([img_batch_size, 4, img_height // 8, img_width // 8], device=device), "noise_mask": torch.zeros([img_batch_size, 1, img_height , img_width], device=device)}
        ...
        dummy_daten = {
            ...
            'img_ts': dummy_img_ts,
            'logits': empty_logits,
            'latent': {"samples": torch.zeros([1, 4, img_height // 8, img_width // 8], device=device), "noise_mask": torch.zeros([1, 1, img_height, img_width ], device=device)}
        }
        ...
        res_images_rgb = torch.cat(res_img_rgb, dim=0).to("cpu")
